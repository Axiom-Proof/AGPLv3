...
class RoomClient {
        ...
        Swal.fire({
            ...
            if (result.value) {
                ...
                if (you_tube_url) {
                    ...
                    let youTubePlayBtn = this.getId('youTubePlayBtn');
                    isYouTubeAutoplay ? hide(youTubePlayBtn) : show(youTubePlayBtn);
                    if (!this.isMySettingsOpen) {
                        this.toggleMySettings();
                        this.getId('tabYoutubeBtn').click();
                    }
                    this.userLog('info', 'YouTube video sent successfully', 'top-end');
                } else {
    ...
    getYoutubeEmbed(url) {
        ...
        return match && match[7].length == 11
            ? 'https://www.youtube.com/embed/' + match[7] + (isYouTubeAutoplay ? '?autoplay=1' : '?autoplay=0')
            : false;
    }
    ...
    youTubeAction(data) {
        ...
        switch (action) {
            case 'open':
                ...
                break;
            case 'play':
                this.userLog('info', `${peer_name} <i class="fab fa-youtube"></i> played the YouTube video`, 'top-end');
                this.playYouTube();
                break;
            case 'close':
    ...
    playYouTube(emit = false, peer_id = 'all') {
        if (emit) {
            let data = {
                peer_id: peer_id,
                peer_name: this.peer_name,
        ...
        this.autoplayYouTube(true);
    }

    autoplayYouTube(autoplay = false) {
        let youTubeIFrame = this.getId('__youTubeIframe');
        if (youTubeIFrame) {
            let iFrameSrc = '';
            if (autoplay) {
                iFrameSrc = youTubeIFrame.src.replace('autoplay=0', 'autoplay=1');
            } else {
                iFrameSrc = youTubeIFrame.src.replace('autoplay=1', 'autoplay=0');
            }
